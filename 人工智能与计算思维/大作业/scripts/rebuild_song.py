...
print(f"Song: {G_song.number_of_nodes()} nodes, {G_song.number_of_edges()} edges")
...
pos = nx.spring_layout(G_vis, k=1.8, iterations=80, seed=42)
...
for i, pid in enumerate(G_vis.nodes()):
    ...
    kw = {"label": name if deg >= 8 else "", "color": color, "size": size, "title": title_h, "x": x, "y": y}
...
html = re.sub(r"<script>\s*var selectedNodeId.*?</script>\s*</head>", "</head>", html, flags=re.DOTALL)
...
js += '            if (neighbors.has(String(e.from)) && neighbors.has(String(e.to))) return Object.assign({}, e, {opacity: 0.8, width: 2});\n'

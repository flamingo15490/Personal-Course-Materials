...
# Add interaction settings to disable node dragging
...
for i, r in enumerate(top_persons_csv[:20]):
    ...
    sidebar += f'<span class="rank-deg">度{deg}</span></div>\n'
...
sidebar += '</div>\n'
sidebar += '<div class="legend"><h4>操作</h4>\n'
sidebar += '<div style="font-size:12px;color:#aaa;line-height:1.6;">点击节点查看关系<br>再点恢复全部<br>点侧边栏跳转</div>\n'
sidebar += '</div></div>\n'
...
# Name map for JS
names_json = json.dumps({str(k): v for k, v in name_map.items()}, ensure_ascii=False)
...
js += '            nodeUpdates.push({id: n.id, label: nm, font: {size: fs, color: "#111", face: "Microsoft YaHei"}, opacity: 1.0});\n'
js += '        } else {\n'
...
js += '    var nodeUpdates = [];\n'
js += '    network.body.data.nodes.forEach(function(n) {\n'
js += '        nodeUpdates.push({id: n.id, label: "", opacity: 1.0});\n'
js += '    });\n'
js += '    network.body.data.nodes.update(nodeUpdates);\n'
js += '    var edgeUpdates = [];\n'
js += '    network.body.data.edges.forEach(function(e) {\n'
js += '        edgeUpdates.push({id: e.id, opacity: 1.0, width: 0.3});\n'
js += '    });\n'
js += '    network.body.data.edges.update(edgeUpdates);\n'
js += '    network.setOptions({physics: false});\n'
...
js += '}\n'
js += 'console.log("Ready, names:", Object.keys(nodeNames).length);\n'
js += '</script>\n'
